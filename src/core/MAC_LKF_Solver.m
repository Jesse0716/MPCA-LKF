function [result] = MAC_LKF_Solver(params)
% MAC_LKF_SOLVER MAC-LKF method solver
% 
% Input:
...
n = params.n;
h_1 = params.h_1;
h_2 = params.h_2_mac;  % Use MAC-LKF specific h_2
h_12 = h_2 - h_1;
h_k = h_1;
h_1k = h_k - h_1 + 1;
h_2k = h_2 - h_k + 1;

alpha_1 = params.alpha_1_mac;  % Use MAC-LKF specific alpha_1
alpha_2 = params.alpha_2_mac;  % Use MAC-LKF specific alpha_2
mu_1 = params.mu_1;
mu_2 = params.mu_2_mac;  % Use MAC-LKF specific mu_2
...
P12=sdpvar(n,n,'symmetric');
P21=sdpvar(n,n,'symmetric');
...
V1_1=v1'*(P11+h_1*Z11+h_12*Z12)*v1; %p=1
V1_2=v1'*(P12+h_1*Z11+h_12*Z12)*v1;

V2_1=v1'*(P21+h_1*Z21+h_12*Z22)*v1; %p=2
V2_2=v1'*(P22+h_1*Z21+h_12*Z22)*v1;
...
V1_111=e_1'*(P11+(b11-a11)/10*P11+(b12-a12)/10*P12)*e_1-(1-alpha_1)*v1'*P11*v1; %p=1
V1_121=e_1'*(P12+(b11-a11)/10*P11+(b12-a12)/10*P12)*e_1-(1-alpha_1)*v1'*P12*v1;

V1_211=e_2'*(P21+(b21-a21)/10*P21+(b22-a22)/10*P22)*e_2-(1-alpha_2)*v1'*P21*v1; %p=2
V1_212=e_2'*(P22+(b21-a21)/10*P21+(b22-a22)/10*P22)*e_2-(1-alpha_2)*v1'*P22*v1;
...
V11_2=(K1*v1-v5)'*M11*(v5-K2*v1)+(K1*v2-v6)'*M12*(v6-K2*v2)+(K1*v3-v7)'*M13*(v7-K2*v3)+(K1*v4-v8)'*M14*(v8-K2*v4)...
    +[(K1*v1-v5)'*M11*(v5-K2*v1)+(K1*v2-v6)'*M12*(v6-K2*v2)+(K1*v3-v7)'*M13*(v7-K2*v3)+(K1*v4-v8)'*M14*(v8-K2*v4)]'...
    +(K1*(v1-v2)-v5+v6)'*D11*(v5-v6-K2*(v1-v2))+(K1*(v2-v3)-v6+v7)'*D12*(v6-v7-K2*(v2-v3))+(K1*(v3-v4)-v7+v8)'*D13*(v7-v8-K2*(v3-v4))...
    +[(K1*(v1-v2)-v5+v6)'*D11*(v5-v6-K2*(v1-v2))+(K1*(v2-v3)-v6+v7)'*D12*(v6-v7-K2*(v2-v3))+(K1*(v3-v4)-v7+v8)'*D13*(v7-v8-K2*(v3-v4))]';
...
V1=V1_111+V1_121+V2_21+V3_31+V4_41+V5_51...
    +V6_61+V7_71+V8_81+V91+V10_1+V11_1+V12_1;%p=1

V2=V1_211+V1_212+V2_22+V3_32+V4_42+V5_52...
    +V6_62+V7_72+V8_82+V92+V10_2+V11_2+V12_2;%p=2
  
V3=V1_1+V1_2+V21+V31;
V4=V2_1+V2_2+V22+V32;

%V5=a121*P121+a122*P122-pho_1*(b111*P111+b111*P112);
%V6=a221*P221+a222*P222-pho_2*(b211*P211+b212*P212);

V7=a11*P11+a12*P12-mu_1*(b21*P21+b22*P22);
V8=a21*P21+a22*P22-mu_2*(b11*P11+b12*P12);
...
F=[P11>=0.0001*eye(n),P12>=0.0001*eye(n),P21>=0.0001*eye(n),P22>=0.0001*eye(n),
...
F=F+F+F+[V1<=-0.0001*eye(20*n),V2<=-0.0001*eye(20*n),V3>=0.0001*eye(20*n),V4>=0.0001*eye(20*n),
     V7<=-0.0001*eye(1*n),V8<=-0.0001*eye(1*n)];
...
if result.feasible
    ...
    result.message = 'MAC-LKF LMI problem solved successfully';
else
    result.status = 'Infeasible';
    result.message = 'MAC-LKF LMI problem infeasible';
end
